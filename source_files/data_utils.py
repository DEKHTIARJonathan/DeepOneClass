...
def get_train_val_imgs(class_nbr, data_dir, target_w):
    ...
    train_images = np.array(list(map(lambda f: get_image(f, target_w=target_w, is_crop=False), data_path_files[:num_train])))
    train_images = np.stack([train_images]*3, axis=3).squeeze(4)
    assert (train_images.shape == (num_train, target_w, target_w, 3))


    test_images = np.array(list(map(lambda f: get_image(f, target_w=target_w, is_crop=False), data_path_files[num_train:] + data_def_path_files)))
