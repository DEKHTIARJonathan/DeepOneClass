...
def cnn_network(inputs):
    ...
    filter1_shape = (5, 5, 1, 20)
    filter2_shape = (256, 256, 20, 10)
    ...
    if 1:
        ...
        l1 = tf.nn.convolution(inputs, filter=filter1, padding="SAME", name="conv1")
        l1_out = tf.nn.relu(l1)
        l2 = tf.nn.convolution(l1_out, filter=filter2, padding="VALID", name="conv1")
        l2_out = tf.nn.tanh(l2)

    # 2.2
    else:
        il = tl.layers.InputLayer(inputs, name="inputs")
        l1 = tl.layers.Conv2dLayer(il, act=tf.nn.relu, shape=filter1_shape, padding="SAME", name="conv1")
        l2 = tl.layers.Conv2dLayer(l1, act=tf.nn.tanh, shape=filter2_shape, padding="VALID", name="conv2")
        l2_out = l2.outputs
